...
def parser_setup():
    ...
    parser.add_argument('--env_id', type=str, default='RoomNavigation-v0')
...
def main(env_id, train, logdir, exploration, env_dt, **kwargs):
    ...
    action_dim = env.action_space.shape[0]  # get the dimesnions of action space
    state_dim = env.observation_space.shape[0] # get the dimensions of state space
    noise = U.OUNoise(action_dim, 0.0, 0.15, 0.2, 0.05, exploration, env_dt) # noise implementation
    print(noise)
    agent = DDPG(sess, state_dim, action_dim, u_bound=u_bound, noise=noise, **ddpg_kwargs) # creating a ddpg agent

    play = U.Play(sess, env, agent, logdir) # play the board

    if train:
        play.train(kwargs['nb_episodes'], kwargs['nb_eval_episodes']) # training 
